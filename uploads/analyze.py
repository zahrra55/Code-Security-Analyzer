...
def analyze_code_vulnerability(code_snippet, language):
    ...
    Returns:
        int: 1 if a vulnerability is detected, 0 if the code is secure, -1 if an error occurs.
    """
    try:
        ...
        vulnerability_score = probabilities[0][1].item()  # Probability of the "vulnerable" class
        logging.info(f"Vulnerability score: {vulnerability_score}")
        return 1 if vulnerability_score > 0.5 else 0  # Adjust threshold as needed
    except Exception as e:
...
def suggest_fix(code_snippet, language):
    ...
    Returns:
        str or None: The fixed code if vulnerabilities are found, otherwise None.
    """
    fixes = {
        'c': {
...
def index():
    ...
    if request.method == "POST":
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template("index.html", execution_time=execution_time, result=prediction, code=code, fixed_code=fixed_code)
